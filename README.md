...
`destiny` is a Claude Code plugin that produces a daily fortune reading from your birth date and time. It's deliberately built so the LLM is **not** the source of the reading — only the translator of one.

How the pipeline is split, and why it matters:

1. **The math is computed locally, not generated by an LLM.** Your birth date / time / longitude go into a Python script that uses [`lunar-python`](https://github.com/6tail/lunar-python) — a perpetual lunar calendar implementing the same arithmetic published by Korea's national astronomical observatory. That script returns your eight-character birth chart, today's "day pillar," and an I-Ching hexagram for the moment. Same person + same day always produces the exact same numbers; you can verify them against any traditional almanac.

2. **The interpretation follows a documented tradition, not freeform invention.** The script's numerical output is the only thing handed to Claude. Claude then writes prose using a fixed, century-old set of reading conventions — Five Elements generation/control cycles, the Ten Gods relationships, branch harmonies and clashes — all encoded into the skill's prompt and lookup tables. This is closer to translating a known input through a documented framework than to making something up.

**To be explicit about what this does and doesn't claim:** Four Pillars and I-Ching are classical metaphysical systems with no scientifically established predictive validity. This tool does not change that. The deterministic-compute design is about *reproducibility and tradition-fidelity*, not accuracy — same inputs always produce the same chart, and the prose follows documented classical rules instead of LLM imagination. Whether the chart "means" anything is a separate question, and the honest answer is that this is for fun, not for life decisions. (See the Safety & disclaimer section below.)

Default output is English, with every classical term unpacked into plain language so a reader with zero exposure to East Asian astrology can follow along. Output switches to Korean / Japanese / Chinese / Spanish on request.
